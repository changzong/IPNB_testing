...
def run_cell(shell, iopub, cell):
    ...
    # wait for finish, maximum 600s
    shell.get_msg(timeout=600)
    outs = []
    while True:
        try:
...
def test_notebook(nb):
    ...
    for ws in nb.worksheets:
        for cell in ws.cells:
            ...
            if failed:
                failures += 1
                print str(failures) + " cells have been mismatched."
                print "Mismatched output: "
                print outs
            else:
                successes += 1
                print str(successes) + " cells have been passed."
            #sys.stdout.write('.')
            print "******************************"
